...
public class TestLabirintoBuilder {
	...
	public void testMonolocale() {
		Labirinto monolocale = labirintoBuilder
				...
				.getLabirinto();
	assertEquals(nomeStanzaIniziale,monolocale.getIniziale().getNome());
	...
	public void testMonolocaleConAttrezzo() {
		...
		assertEquals(nomeStanzaIniziale,monolocale.getIniziale().getNome());
		assertEquals(nomeStanzaIniziale,monolocale.getStanzaVincente().getNome());
		assertEquals("spada",monolocale.getIniziale().getAttrezzo("spada").getNome());
	...
	public void testMonolocaleConAttrezzoSingoloDuplicato() {
		Labirinto monolocale = labirintoBuilder
				...
				.addAttrezzo("spada",1)
				.getLabirinto();
		int size = monolocale.getIniziale().getAttrezzi().size();
		assertTrue(size==1);
		assertEquals(Arrays.asList(new Attrezzo("spada",1)),monolocale.getIniziale().getAttrezzi());
	}
	...
	public void testBilocale() {
		Labirinto bilocale = labirintoBuilder
				...
				.getLabirinto();
		assertEquals(bilocale.getStanzaVincente(),bilocale.getIniziale().getStanzaAdiacente("nord"));
		assertEquals(Collections.singletonList("nord"),bilocale.getIniziale().getDirezioni());
	...
	public void testTrilocale(){
		Labirinto trilocale = labirintoBuilder
				...
				.getLabirinto();	
		assertEquals(nomeStanzaIniziale, trilocale.getIniziale().getNome());
		assertEquals(nomeStanzaVincente, trilocale.getStanzaVincente().getNome());
		assertEquals("biblioteca",trilocale.getIniziale().getStanzaAdiacente("sud").getNome());
	}
	...
	public void testTrilocaleConStanzaDuplicata() {
				labirintoBuilder
				.addStanzaIniziale(nomeStanzaIniziale)
				.addStanzaGenerica("stanza generica")
				.addStanzaGenerica("stanza generica")
	...
	public void testPiuDiQuattroAdiacenti() {
		Labirinto maze = labirintoBuilder
				.addStanzaIniziale(nomeStanzaIniziale)
				.addStanzaGenerica("stanza 1")
				.addStanzaGenerica("stanza 2")
				.addStanzaGenerica("stanza 3")
				.addStanzaGenerica("stanza 4")
				.addStanzaGenerica("stanza 5")
		...
		assertNull(maze.getIniziale().getStanzaAdiacente("nord-est"));
		assertTrue(maze.getIniziale().getMapStanzeAdiacenti().size()<=4);
		assertTrue(!maze.getIniziale().getMapStanzeAdiacenti().containsValue(test));
		...
		assertEquals(mappa,maze.getIniziale().getMapStanzeAdiacenti());
	}
	...
	public void testImpostaStanzaInizialeCambiandola() {
		Labirinto maze = labirintoBuilder
				...
				.addStanzaGenerica("nuova iniziale")
				.addStanzaIniziale("nuova iniziale")
				.getLabirinto();
		assertEquals("nuova iniziale",maze.getIniziale().getNome());
	}
	...
	public void testAggiuntaAttrezziAStanze_Iniziale() {
		...
		assertEquals(attrezzo,maze.getIniziale().getAttrezzo(nomeAttrezzo));
	}
	...
	public void testLabirintoCompletoConTutteLeStanze() {
		...
		Labirinto labirintoCompleto = this.labirintoBuilder
				...
				.getLabirinto();
		assertEquals(nomeStanzaIniziale, labirintoCompleto.getIniziale().getNome());
		assertEquals(nomeStanzaVincente, labirintoCompleto.getStanzaVincente().getNome());
		Stanza corridoio = labirintoCompleto.getIniziale().getStanzaAdiacente("nord");
